...
//                .on(team.name.eq("teamA"))
                .where(team.name.eq("teamA")) // innerJoin과 같음
                .fetch();
    ...
    public void join_on_no_relation() {
        ...
        List<Tuple> result = queryFactory
                .select(member, team)
                .from(member)
                .leftJoin(team) // => id로 매칭 X, 이름으로만 매칭
//                .leftJoin(member.team, team) => id로 매칭
    ...
    /**
     * JPA JPQL 서브쿼리의 한계점 : from 절의 서브쿼리(인라인 뷰) 지원 X
     * 해결방안
     * 1. 서브쿼리를 join으로 변경
     * 2. 쿼리를 2번 분리해서 실행
     * 3. nativeSQL 사용
    ...
    /**
     * 가급적이면 DB에서는 최소한의 일만 (필터링, 그룹핑하고 데이터를 줄이는 일)
     * 데이터를 수정하는 건 애플리케이션에서 처리
     */
    @Test
    ...
    // Tuple은 repository, dao 계층에서만 사용하는게 좋음
    @Test
    ...
    void findDtoByJPQL() throws Exception {
        ...
        // DTO의 package이름 다 적어줘야함, 생성자 방식만 지원
    ...
    // 단점
    // 1. Q파일 생성
    // 2. Dto에 QueryDsl 의존성을 갖게됨 (여러 계층이 Dto 사용), 아키텍쳐 적인 문제
    @Test
...
//                .where(usernameEq(usernameCond), ageEq(ageCond))
    ...
    /**
     * 쿼리 자체의 가독성 높아짐
     * 코드 조립 가능 (메서드를 다른 쿼리에서도 재활용)
     * ex) 광고상태 isValid, 날짜가 In : isServiceable
     */
    ...
    // 수정, 삭제 벌크 연산 (쿼리 한번으로 대량 데이터 수정)
    @Test
    void bulkUpdate() throws Exception {
        // DB와 영속성 컨텍스트의 싱크가 안맞음
